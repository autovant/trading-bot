...
class OrderBookIndicators:
    ...
    def detect_liquidity_walls(
        orderbook: Dict[str, Any], depth: int = 10, wall_multiplier: float = 3.0
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            avg_bid_size = (
                np.mean([level[1] for level in bids_depth]) if bids_depth else 0
            )
            avg_ask_size = (
                np.mean([level[1] for level in asks_depth]) if asks_depth else 0
            )
