...
async def fetch_historical_data(
    ...
    config = ExchangeConfig(
        provider="bybit", name="bybit", testnet=False
    )  # Default to bybit for data
    ...
    try:
        ...
        logger.info(
            f"Fetching data for {symbol} from {start_date} to {end_date} using CCXT..."
        )
        ...
        while current_start < end_date:
            ...
            try:
                ...
                # So I should use client.exchange.fetch_ohlcv
                ...
                ohlcv = await client.exchange.fetch_ohlcv(
                    symbol, interval, since=since_ts, limit=limit
                )
                ...
                chunk["timestamp"] = pd.to_datetime(
                    chunk["timestamp"], unit="ms", utc=True
                )
                ...
                rate_limit = getattr(client.exchange, "rateLimit", None)
                await asyncio.sleep(rate_limit / 1000.0 if rate_limit else 0.1)
...
async def run_backtest(args):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        replay_client = ReplayZoomexClient(
            df, session, initial_capital=args.initial_capital
        )
        ...
        for i, current_time in enumerate(timestamps):
            ...
            if i % 100 == 0:
                ...
                logger.info(
                    f"Progress: {pct:.1f}% - Time: {current_time} - Equity: ${equity:.2f}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="ISO format e.g. 2023-01-01T00:00:00",
    )
