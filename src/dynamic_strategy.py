...
from .models import MarketRegime, TradingSetup, TradingSignal, ConfidenceScore
...
class DynamicStrategyEngine:
    ...
    def _calculate_indicators(self, data: pd.DataFrame, indicators_config: List[IndicatorConfig]) -> pd.DataFrame:
        ...
        for ind in indicators_config:
            try:
                if ind.name == "ema":
                    ...
                    u, m, l = self.indicators.bollinger_bands(df["close"], period, std)
                    df["bb_upper"] = u
                    df["bb_middle"] = m
                    df["bb_lower"] = l
                    df["bb_width"] = (u - l) / m
                    ...
                    if oscillator not in df.columns:
                        ...
                        logger.warning(f"Oscillator {oscillator} not found for divergence check.")
                        continue
                    ...
                    price_low = df["low"]
                    price_high = df["high"]
                    osc_data = df[oscillator]
                    
                    # Find all pivots
                    pivots_low = self.indicators.find_pivots(price_low, lookback)
                    pivots_high = self.indicators.find_pivots(price_high, lookback)
                    pivots_osc_low = self.indicators.find_pivots(osc_data, lookback)
                    pivots_osc_high = self.indicators.find_pivots(osc_data, lookback) # Same for osc usually
                    ...
                    divs = self.indicators.detect_divergence(df["low"], df[oscillator], lookback)
                    # We also need high for bearish
                    divs_high = self.indicators.detect_divergence(df["high"], df[oscillator], lookback)
                    ...
                    if divs["regular_bullish"]: df.iloc[-1, df.columns.get_loc(f"{base_name}_reg_bull")] = 1.0
                    if divs["hidden_bullish"]: df.iloc[-1, df.columns.get_loc(f"{base_name}_hid_bull")] = 1.0
                    
                    # Bearish checks (using Highs)
                    if divs_high["regular_bearish"]: df.iloc[-1, df.columns.get_loc(f"{base_name}_reg_bear")] = 1.0
                    if divs_high["hidden_bearish"]: df.iloc[-1, df.columns.get_loc(f"{base_name}_hid_bear")] = 1.0
    ...
    def _evaluate_condition(self, row: pd.Series, condition: ConditionConfig) -> bool:
        ...
        try:
            # Get values
            val_a = row.get(condition.indicator_a) if isinstance(condition.indicator_a, str) and condition.indicator_a in row else condition.indicator_a
            val_b = row.get(condition.indicator_b) if isinstance(condition.indicator_b, str) and condition.indicator_b in row else condition.indicator_b
            ...
            if isinstance(val_a, str) and val_a in row: val_a = row[val_a]
            if isinstance(val_b, str) and val_b in row: val_b = row[val_b]

            # Convert to float if possible for comparison
            try: val_a = float(val_a)
            except: pass
            try: val_b = float(val_b)
            except: pass
    ...
    def detect_regime(self, data: pd.DataFrame) -> MarketRegime:
        ...
        bullish = all(self._evaluate_condition(current, c) for c in self.config.regime.bullish_conditions) if self.config.regime.bullish_conditions else False
        bearish = all(self._evaluate_condition(current, c) for c in self.config.regime.bearish_conditions) if self.config.regime.bearish_conditions else False
    ...
    def detect_setup(self, data: pd.DataFrame) -> TradingSetup:
        ...
        bullish = all(self._evaluate_condition(current, c) for c in self.config.setup.bullish_conditions) if self.config.setup.bullish_conditions else False
        bearish = all(self._evaluate_condition(current, c) for c in self.config.setup.bearish_conditions) if self.config.setup.bearish_conditions else False
    ...
    def generate_signals(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        for sig_config in self.config.signals:
            ...
            entry_met = all(self._evaluate_condition(current, c) for c in sig_config.entry_conditions)
            
            print(f"Checking signal {sig_config.signal_type}: entry_met={entry_met}")
            ...
            if entry_met:
                ...
                if self.config.risk.take_profit_type == "risk_reward" and stop_loss > 0:
                    ...
                    tp_dist = current_price * (self.config.risk.take_profit_value / 100.0)
                    if sig_config.direction == "long":
    ...
    def calculate_confidence(self, regime: MarketRegime, setup: TradingSetup, signal: TradingSignal) -> ConfidenceScore:
        """Calculate confidence score."""
        # Simple weighted sum
        regime_score = regime.confidence * self.config.regime.weight * 100
        setup_score = setup.strength * self.config.setup.weight * 100
        signal_score = signal.strength * 0.35 * 100 # Default weight for signal if not in config
