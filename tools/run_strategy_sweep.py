...
from tools.backtest_perps import PerpsBacktest, CsvDataProvider
...
class StrategySweeper:
    ...
    async def run_sweep(self, smoke_test: bool = False) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for symbol in self.symbols:
            ...
            try:
                ...
                htf_df = ltf_df.resample("60min").agg({
                    "open": "first",
                    "high": "max",
                    "low": "min",
                    "close": "last",
                ...
                logger.info(f"Loaded data for {symbol}: LTF={len(ltf_df)} rows, HTF={len(htf_df)} rows")
                if ltf_df.empty:
            ...
            for i, params in enumerate(grid):
                ...
                try:
                    ...
                    if "error" not in metrics:
                        ...
                        logger.warning(f"Backtest returned error for {symbol}: {metrics['error']}")
                except Exception as e:
    ...
    def analyze_and_report(self, all_results: Dict[str, List[Dict[str, Any]]]):
        ...
        for symbol, results in all_results.items():
            ...
            for name, config in profiles.items():
                if config:
                    m = config["metrics"]
                    risk_pct = 0.002 if name == "Conservative" else (0.003 if name == "Standard" else 0.004)
                    ...
                    # Generate YAML
                    self._generate_yaml_profile(symbol, name, config["params"], risk_pct)
                else:
                    report_lines.append(f"| {symbol} | {name} | N/A | - | - | - | - | - |")
            ...
            sorted_configs = sorted(valid_configs, key=lambda x: self._calculate_score(x), reverse=True)
            for i, cfg in enumerate(sorted_configs[:3]):
                m = cfg["metrics"]
                p_str = ", ".join(f"{k}={v}" for k, v in cfg["params"].items() if k in ["atrStopMultiple", "tp2Multiple", "maxBarsInTrade"])
                report_lines.append(
    ...
    def _select_profile(self, configs: List[Dict[str, Any]], profile_type: str) -> Optional[Dict[str, Any]]:
        candidates = []
        for c in configs:
            m = c["metrics"]
            ...
            if profile_type == "conservative":
                if dd <= 10 and trades >= (40 if SWEEP_MODE == "production" else 15) and 0.6 <= avg_r <= 1.5:
                    candidates.append(c)
    ...
    def _generate_yaml_profile(self, symbol: str, profile_name: str, params: Dict[str, Any], risk_pct: float):
...
async def main():
    ...
    parser.add_argument("--csv-dir", default="data/history", help="Directory containing CSV files")
    parser.add_argument("--output-dir", default="results", help="Directory for results")
    parser.add_argument("--analyze-only", action="store_true", help="Skip backtest and only analyze existing results")
    parser.add_argument("--mode", choices=["production", "exploration"], default="production", help="Sweep mode")
