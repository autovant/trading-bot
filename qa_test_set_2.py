...
def create_strategy():
    ...
    resp = requests.post(f"{BASE_URL}/strategies", json={"name": "QA_AlwaysLong", "config": strategy_config})
...
def create_synthetic_data():
    ...
    prices_1h = [10000 + i * 2 for i in range(hours)] # Slower growth to keep prices reasonable
    df_1h = pd.DataFrame({
        "timestamp": dates_1h,
        "open": prices_1h,
    ...
    # 4H Data
    dates_4h = [datetime(2023, 1, 1) + timedelta(hours=i*4) for i in range(hours // 4)]
