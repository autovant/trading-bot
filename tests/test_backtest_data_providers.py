...
async def test_csv_data_provider_parses_file(tmp_path: Path):
    ...
    rows = [
        {"timestamp": 1700000000000, "open": 10, "high": 11, "low": 9, "close": 10.5, "volume": 100},
        {"timestamp": 1700000300000, "open": 10.5, "high": 11.5, "low": 10, "close": 11, "volume": 120},
    ]
    header = "timestamp,open,high,low,close,volume\n"
    with csv_path.open("w", encoding="utf-8") as handle:
        handle.write(header)
        for row in rows:
            handle.write(",".join(str(row[k]) for k in ["timestamp", "open", "high", "low", "close", "volume"]))
            handle.write("\n")

    provider = CsvDataProvider(csv_path=str(csv_path))
    df = await provider.fetch("SOLUSDT", "5", start_date="2023-11-01", end_date="2023-12-01")
