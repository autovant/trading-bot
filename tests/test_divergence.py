...
from src.dynamic_strategy import DynamicStrategyEngine, StrategyConfig, RegimeConfig, SetupConfig, SignalConfig, RiskConfig, IndicatorConfig, ConditionConfig
...
class TestDivergence(unittest.TestCase):
    def test_detect_divergence(self):
        ...
        self.assertFalse(results["regular_bearish"]) # Highs are 100, 105, 110, 115 (Higher), Ind 50, 60, 70, 80 (Higher). No div.
    ...
    def test_engine_integration(self):
            ...
            signals=[
                SignalConfig(
                    indicators=[
                        ...
                        IndicatorConfig(name="divergence", params={"oscillator": "rsi_2", "lookback": 1})
                    ],
                    entry_conditions=[
                        ConditionConfig(indicator_a="rsi_2_div_reg_bull", operator="==", indicator_b=1.0)
                    ]
                )
            ],
            risk=RiskConfig()
        ...
        data = pd.DataFrame({
            ...
            "close": [100, 95, 105, 90, 110, 85, 115], # Close follows low for RSI calc simplicity
            "volume": [1000]*7
        })
