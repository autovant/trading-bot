...
async def _build_service(monkeypatch, warning_size_multiplier=1.0):
    config = PerpsConfig(
    ...
    await db.initialize()
    service.database = db
    service.intent_ledger = OrderIntentLedger(
        database=db, mode="paper", run_id="runtime-health"
    )
...
async def test_runtime_health_ok_allows_orders(monkeypatch, tmp_path):
    service, enter_mock, client = await _build_service(monkeypatch)
...
async def test_runtime_health_warning_scales_size(monkeypatch, tmp_path):
    service, enter_mock, _ = await _build_service(monkeypatch, warning_size_multiplier=0.5)
    store = SymbolHealthStore(tmp_path / "health.json")
    store.update_symbol_state(
