...
class TestDynamicStrategyEngine(unittest.TestCase):
    def setUp(self):
        ...
        self.config = StrategyConfig(
            ...
            regime=RegimeConfig(
                ...
                bullish_conditions=[
                    ConditionConfig(
                        indicator_a="close", operator=">", indicator_b="ema_10"
                    )
                ],
                bearish_conditions=[
                    ConditionConfig(
            ...
            signals=[
                SignalConfig(
                    ...
                    entry_conditions=[
                        ConditionConfig(
                            indicator_a="close", operator=">", indicator_b=100
                        )
                    ],
    ...
    def test_setup_detection(self):
        ...
        self.config.setup.bullish_conditions = [
            ConditionConfig(indicator_a="close", operator="<", indicator_b=90)
        ]
        data = pd.DataFrame(
            {
                "close": [85] * 20,
